...
-- 2017-01-01,CRauch,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name
From SysDatabases
Where Name = 'Assignment07DB_CRauch')
	 Begin
	Alter Database [Assignment07DB_CRauch] set Single_user With Rollback Immediate;
	Drop Database Assignment07DB_CRauch;
End
	Create Database Assignment07DB_CRauch;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_CRauch;
...
FROM vProducts as vP
	JOIN vCategories as vC ON vC.CategoryID = vP.CategoryID
...
AS
	SELECT TOP 10000000000
		vP.ProductName,
		InventoryDate = DATENAME(mm,vI.InventoryDate) + ', ' + DATENAME(YEAR,vI.InventoryDate),
...
AS
	...
	FROM vCategories as vC
		JOIN vProducts as vP ON vP.CategoryID = vC.CategoryID
		JOIN vInventories as vI ON vI.ProductID = vP.ProductID
	GROUP BY CategoryName, vI.InventoryDate
...
AS
	SELECT TOP 10000000
		...
		PreviousMonthCount = ISNULL(
			LAG(vPI.InventoryCount) OVER ( 
			PARTITION BY vPI.ProductName
			ORDER BY CAST ('01' + vPI.InventoryDate AS DATE)
		),
		0
	)
	FROM vProductInventories as vPI
	ORDER BY ProductName, CAST('01' + vPI.InventoryDate AS DATE);
go
...
AS
	SELECT TOP 10000000000
		vPIPM.ProductName,
		vPIPM.InventoryDate,
		vPIPM.InventoryCount,
		vPIPM.PreviousMonthCount,
		CountVsPreviousCountKPI = 
								CASE
									WHEN vPIPM.InventoryCount > vPIPM.PreviousMonthCount THEN 1
									WHEN vPIPM.InventoryCount = vPIPM.PreviousMonthCount THEN 0
									WHEN vPIPM.InventoryCount < vPIPM.PreviousMonthCount THEN -1
								END
	FROM vProductInventoriesWithPreviousMonthCounts as vPIPM
	ORDER BY vPIPM.ProductName, CAST('01' + vPIPM.InventoryDate AS DATE);
GO
...
CREATE FUNCTION dbo.fProductInventoriesWithPreviousMonthCountsWithKPIs(@KPI INT)
	...
	RETURN (
			SELECT TOP 10000000000
				...
				vPIPM.InventoryCount,
				vPIPM.PreviousMonthCount,
				vPIPM.CountVsPreviousCountKPI
	FROM vProductInventoriesWithPreviousMonthCountsWithKPIs as vPIPM
	WHERE vPIPM.CountVsPreviousCountKPI = @KPI
	ORDER BY vPIPM.ProductName, CAST('01' + vPIPM.InventoryDate AS DATE)
);
go
